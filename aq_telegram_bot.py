...
lines.append(”\n_↓ means signal downgraded by regime filter*”)
lines.append(”*/a TICKER for full analysis + chart*”)
send(chat_id, “\n”.join(lines))
...
blocked,block_reasons,*,*,*=check_portfolio_risk(ticker,sector or “—”)
...
msg+=(f”\n_T1 hit → move SL to breakeven ({entry:.2f})*\n”
f”*T2 hit → trail SL to T1 ({t1:.2f} if available)*\n”
f”*/close\*{lid} <price> to exit_”)
send(chat_id,msg)
...
f”*After T1: move SL to {cs}{entry:.2f} (breakeven)*\n”
f”*After T2: trail SL to T1*\n”
f”*Max open trades: {MAX_OPEN_TRADES} │ Max sector: {MAX_SECTOR_TRADES}*”)
except Exception as e: send(chat_id,f”❌ Error: {e}”)
...
f”{detail}\n\n”
f”📋 {advice[regime]}\n\n”
f”*Regime updates hourly. Signal strength is adjusted automatically.*”)
